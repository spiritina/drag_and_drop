...
function drawSerial(serial){
    let {data} = serial;
    let{Actors, Awards, Country, Director, Genre, Language, Plot, Poster, imdbRating, Title, totalSeasons} = data;
    ...
    let gradient = createGradient(svg[Math.floor(imdbRating/2)].parentNode, 'grad1',[{'offset':`0%`, 'stop-color':'yellow'},
  {'offset':`${(imdbRating%2)/2*100}%`,'stop-color':'yellow'},{'offset':`${(imdbRating%2)/2*100}%`,'stop-color':'#424242'}]);
